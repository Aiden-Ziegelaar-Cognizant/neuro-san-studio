...
class GetDevicesBySite(CodedTool):
    def __init__(self):
    ...
    def invoke(self, args: Dict[str, Any], sly_data: Dict[str, Any]) -> Union[Dict[str, Any], str]:
        ...
        self.tailings_db.execute(f"SELECT device_name FROM vwp_devices WHERE datalogger_id IN (SELECT datalogger_id FROM dataloggers WHERE site_id = (SELECT site_id FROM sites WHERE site_name= ?))", [site])
